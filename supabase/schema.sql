...
create table if not exists public.plannings (
  ...
  meal_multipliers jsonb not null default '{}',
  created_at timestamptz default now()
);
...
-- Table des préférences planning (profil nutritionnel utilisateur)
...
create index if not exists idx_user_planning_preferences_user on public.user_planning_preferences(user_id);
